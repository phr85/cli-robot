...
function parseKompendium(cfg) {
  return new Promise(function (resolve, reject) {
    ...
    var items = fs.createWriteStream(cfg.process.catalog);
    ...
    function writePart(data, tag, path)
    {
      ...
      {
        var file = cfg.process.dir + "/" + item.lang + "/" +item.type + "/" +element + ".htm";

        item.authNrs = (item.authNrs) ? item.authNrs+ " "+element : element;

        if (!writtenFiles[file]) {
    ...
    xs.on('end', function(counter)
    {
      items.on("finish", function()
      {
        ...
        var liste = JSON.parse( fs.readFileSync(cfg.process.catalog));
