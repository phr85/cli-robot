...
function parseKompendium(filename) {
  return new Promise(function (resolve, reject) {
    var xs = new splitter('/medicalInformations/medicalInformation');

    var items = fs.createWriteStream(pathResolve(__dirname, "../../data/release/kompendium/catalog.json"));
    ...
    function writePart(data, tag, path)
    {
      ...
      {
        var file = pathResolve(__dirname, "../../data/release/kompendium/"+item.lang+"/"+item.type+"/"+element+".htm")

        item.authNrs = (item.authNrs) ? item.authNrs+ " "+element : element;

        log.doing( "Kompendium", "Files #", done++ );

        fs.writeFile(file, repairHTML( data ));
      });
    ...
    xs.on('end', function(counter)
    {
      items.on("finish", function()
      {
        ...
        var liste = JSON.parse( fs.readFileSync(pathResolve(__dirname, "../../data/release/kompendium/catalog.json" )));
    ...
    var streamIn = fs.createReadStream(filename, { start:3 });
