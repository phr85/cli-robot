...
var fileWriter = {
   ...
   * @returns {Promise}
   */
  unzip: function (zipname, zipFiles) {
    var self = this;
    ...
    return new Promise(function (resolve, reject) {
      ...
      readStream.pipe(unzip.Parse()).on("entry", function (entry) {
        var dest = self._match(zipFiles, entry.path);
        if (!dest) {
          entry.autodrain();
        } else {
  ...
  write: {
     ...
     *
     * @param {String} filename
     * @param {Stream} source - e.g. a Download/Response
     * @param {Function} renderStatus
     * @returns {Promise}
     */
    stream: function (filename, source, renderStatus) {
      return new Promise(function (resolve, reject) {
        var writeStream = fs.createWriteStream(filename, {encoding: "utf8"});

        if (renderStatus) {
          source.on("data", renderStatus);
        }
