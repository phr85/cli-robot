...
var createDataStore = require('../common/createDataStore');
var resolveMissingHistoryFile = require("../common/resolveMissingHistoryFile");
 ...
 * @param {function()} onDeRegistered
 * @param {function()} onNew
 * @param {function(null|Error)?} done - optional
 * @param {Log|console?} log - optional
 * @returns {Promise}
 */
function history(jobName, cfg, onChanged, onDeRegistered, onNew, done, log) {
  ...
  return disk.ensureDir(cfg.history.dir, cfg.history.log.dir)
    ...
    .then(function (result) {
      ...
      log.time(jobName, "Create Entry Collection");
    ...
    .catch(resolveMissingHistoryFile(
      jobName,
      cfg.history.file,
      cfg.process.file,
      cfg.process.minFile,
