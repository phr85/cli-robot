...
function readXLSX(filename, corrector) {
  ...
  for( var i = 5; i < 20000; i++ )
  {
    ...
    // IT-Nummer
    if( excel.Sheets[ worksheet ]["F"+i] ) clean.itnummer = excel.Sheets[ worksheet ]["F"+i].v;

    // ATC-Code
    if( excel.Sheets[ worksheet ]["G"+i] ) clean.atc = excel.Sheets[ worksheet ]["G"+i].v;

    // Heilmittelcode
    if ( excel.Sheets[ worksheet ]["E"+i] ) clean.heilmittelcode = excel.Sheets[ worksheet ]["E"+i].v;
    ...
    // Wirkstoffe
    if( excel.Sheets[ worksheet ]["Q"+i]) clean.wirkstoffe = excel.Sheets[ worksheet ]["Q"+i].v;
    // Zusammensetzung
    if( excel.Sheets[ worksheet ]["R"+i] )
      clean.zusammensetzung = excel.Sheets[ worksheet ]["R"+i].v;
    // Anwendungsgebiet Präparat
    if( excel.Sheets[ worksheet ]["S"+i] ) clean.anwendungsgebiet = excel.Sheets[ worksheet ]["S"+i].v;
    // Anwendungsgebeit Sequenz
    if( excel.Sheets[ worksheet ]["T"+i] ) clean.anwendungsgebietsequenz = excel.Sheets[ worksheet ]["T"+i].v;
